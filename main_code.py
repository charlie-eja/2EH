...
def simulation_pca(data_title,x,y=None,components=2,plot_samping=-1,save_or_not=0):
    ...
    if components==2:
        ...
        PCA_plot.pca_plot_3D_variable_vector_html(pca_vector=pca_vector,pca_variance_ratio=pca_variance_ratio,
                                                  vector_name=data_title)
def simulation_umap(x,y=None,components=2,plot_samping=-1):
    # x, y = load_iris(return_X_y=True)
    normalize_data, mean_data, std_data = preprocessing.normalize_gaussian(data=x)
...
def simulation_tsne(x,y=None,components=2,plot_samping=-1):
    # x, y = load_iris(return_X_y=True)
    normalize_data, mean_data, std_data = preprocessing.normalize_gaussian(data=x)
...
def main():
    try:
        ...
        interval_data ,data_lengths= preprocessing.multi_time_sampling(
            ...
            time_index='Time',)
        # plt.figure()
        # plt.plot(interval_data[:,0],'b-',label='Time')
        # plt.title('ML2EH_FI-165-1.PV')
        # int('d')
